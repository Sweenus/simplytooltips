...
public class TooltipPainter {
    ...
    public static void drawItemFrameProgress(DrawContext context, int x, int y, int size,
        ...
        float revealT = Math.clamp(tooltipElapsedMs / (float) ITEM_FRAME_REVEAL_MS, 0.0F, 1.0F);
    ...
    private static void drawFrameProgressContour(DrawContext context, List<int[]> path,
        ...
        int targetLength = Math.clamp(
                Math.round(path.size() * progress.fraction()), 0, path.size());
        int displayedLength = Math.clamp(
                Math.round(targetLength * revealT), 0, targetLength);
