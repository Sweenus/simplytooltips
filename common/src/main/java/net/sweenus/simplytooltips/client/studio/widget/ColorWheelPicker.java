...
public final class ColorWheelPicker {
    ...
    private static final Identifier WHEEL_TEXTURE = new Identifier(SimplyTooltips.MOD_ID, "studio/color_wheel");
    ...
    private static void ensureWheel(MinecraftClient client) {
        ...
        for (int py = 0; py < WHEEL; py++) {
            for (int px = 0; px < WHEEL; px++) {
                ...
                image.setColor(px, py, ColorHelper.Abgr.getAbgr(
                        ColorHelper.Argb.getAlpha(argb), ColorHelper.Argb.getBlue(argb),
                        ColorHelper.Argb.getGreen(argb), ColorHelper.Argb.getRed(argb)));
            }
        }
